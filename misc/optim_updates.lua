...
function grad_update(params, grad_params, learning_rate, optim, opt)
  -- perform a parameter update
  if optim == 'rmsprop' then
    rmsprop(params, grad_params, learning_rate, opt.optim_alpha, opt.optim_epsilon, opt.optim_state)
  elseif optim == 'adagrad' then
    adagrad(params, grad_params, learning_rate, opt.optim_epsilon, opt.optim_state)
  elseif optim == 'sgd' then
    sgd(params, grad_params, learning_rate)
  elseif optim == 'sgdm' then
    sgdm(params, grad_params, learning_rate, opt.optim_alpha, opt.optim_state)
  elseif optim == 'sgdmom' then
    sgdmom(params, grad_params, learning_rate, opt.optim_alpha, opt.optim_state)
  elseif optim == 'adam' then
    adam(params, grad_params, learning_rate, opt.optim_alpha, opt.optim_beta, opt.optim_epsilon, opt.optim_state)
  else
    error('error: bad option optim.')
  end
end